...
%   p = ones(2*N,1);
%   p(1) = sqrt(2);
%   f = (p * p') .* f2; %frequency content
  
%   tx = ifft2(f) * sqrt(prod(size(f)));
  
%   %call curvelet transform
...
%   tc = fdct_wrapping_mex(m,n,nbscales,nbangles_coarse,allcurvelets,double(tx));
%   rmpath /Users/lexing/projects/curve_lab/CurveLab-2.0.2/fdct_wrapping_cpp/mex;
  
%   c = cell(1,length(tc));
%   for s=2:length(tc)
%     nw = length(tc{s});
%     c{s} = tc{s}(1+3/8*nw:5/8*nw);
%   end
%   tmp0 = fft2(tc{1}{1}) / sqrt(prod(size(tc{1}{1})));
%   [m,n] = size(tmp0);
%   mp = (m+1)/2;  np = (n+1)/2;
%   tmp = tmp0(1:mp,1:np);
%   q = ones(mp,1);
